...
We have added html `<meta>` tags to the site, these are currently **_very_** basic, but should improve over the coming week! When you share in Slack, Twitter, Facebook, etc. you will now have links with a bit more meta data! 🏷️
...
- Selecting a link to open in a new page now no longer _also_ selects the parent node in the editor (which had a blue outline).
...
- Dates now do not have a border when _not_ in edit mode
